...
def get_columns(filters: Filters) -> List[Dict]:
	...
	if filters.summarized_view:
		...
		columns.append({"label": _("Shift"), "fieldname": "shift", "fieldtype": "Data", "width": 110})
...
def get_columns_for_days(filters: Filters) -> List[Dict]:
	...
	for i in range(total_days + 1):
		...
		label = "{} {}".format(date.day, weekday)
		date_string = "{:04d}-{:02d}-{:02d}".format(date_list[i].year, date_list[i].month, date_list[i].day)
		days.append({"label": label, "fieldtype": "Data", "fieldname": date_string, "width": 65})

	return days

def get_date_list(filters):
	date_list = []
	start_date = datetime.strptime(filters.from_date, "%Y-%m-%d")
...
def get_total_days(filters) -> int:
	start_date = datetime.strptime(filters.from_date, "%Y-%m-%d")
	end_date = datetime.strptime(filters.to_date, "%Y-%m-%d")
...
def get_attendance_status_for_detailed_view(
	...
	for shift, status_dict in employee_attendance.items():
		...
		for day in range(total_days + 1):
			date = date_list[day].date()
			date_string = "{:04d}-{:02d}-{:02d}".format(date.year, date.month, date.day)
			status = status_dict.get(date)
			if status is None and holidays:
				status = get_holiday_status(date, holidays)

			abbr = status_map.get(status, "")
			row[date_string] = abbr
